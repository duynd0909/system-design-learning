      ...
      >
        ...
        {result.xpBreakdown && (
          <p className="mt-1 text-xs text-[var(--text-secondary)]">
            {[
              result.xpBreakdown.base > 0 &&
                `+${result.xpBreakdown.base} first pass`,
              result.xpBreakdown.attempt > 0 &&
                `+${result.xpBreakdown.attempt} attempt`,
              result.xpBreakdown.streakBonus > 0 &&
                `+${result.xpBreakdown.streakBonus} streak bonus`,
            ]
              .filter(Boolean)
              .join(' · ')}
          </p>
            ...
            <>
              {onDismiss && (
                <Button
                  type="button"
                  onClick={onDismiss}
                  className="bg-[#00ffa3] text-black font-bold hover:bg-[#00ffa3]/90 shadow-[0_4px_16px_rgba(0,255,163,0.25)]"
                >
                  Edit Answers
                </Button>
              )}
            ...
            <div className="space-y-2">
              {result.slotResults.map((slot) => {
                const submittedLabel = slot.submitted
                  ? (componentBySlug.get(slot.submitted)?.label ??
                    slot.submitted)
                  : null;
                const expectedLabel =
                  componentBySlug.get(slot.expected)?.label ?? slot.expected;
                return (
                  <div
                    key={slot.slotId}
                    className={cn(
                  ...
                  >
                    {slot.correct ? (
                      <CheckCircle2
                        className="mt-0.5 h-4 w-4 shrink-0 text-[var(--slot-correct)]"
                        aria-hidden="true"
                      />
                    ) : (
                      <XCircle
                        className="mt-0.5 h-4 w-4 shrink-0 text-[var(--slot-incorrect)]"
                        aria-hidden="true"
                      />
                    )}
                    <div>
